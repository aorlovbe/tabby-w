...
class Counters {
    static find(req, callback) {
        ...
        redis.hget(profiles + req.body.game_id + ":profiles", md5.md5(req.user.id), function (err, profile) {
            if (err || profile === null) {
                ...
                redis.hgetall(counters + profile + ":counters", function (err, result) {
                    if (err || result === null) {
                        log.info('There are no counters found for:', req.user.id, profile);
    ...
    static findbyprofile(req, callback) {
        ...
        redis.hgetall(counters + req.body.profile_id + ":counters", function (err, result) {
            if (err || result === null) {
    ...
    static create(req, callback) {
        redis.multi()
            .hset(counters + req.body.profile_id + ":counters", req.body.name, req.body.value)
            .exec(function (err, result) {
            if (err) {
                log.error('Counter cannot be stored:', err, req.body.profile_id, req.body);
                return callback(true);
            } else {
                log.info('Counter is created:', req.body.name, req.body.value, result[0]);
    ...
    static remove(req, callback) {
        ...
        redis.multi()
            ...
            .exec(function (err) {
                if (err) {
                    log.error('Counter cannot be deleted:', err, req.body.profile_id, req.body);
                    return callback(true);
                } else {
    ...
    static modify(req, callback) {
        log.info('Searching counters by profile ID:', req.body.profile_id);
        redis.multi()
            .hincrby(counters + req.body.profile_id + ":counters", req.body.name, req.body.value)
            .exec(function (err, result) {
                if (err) {
                    log.error('Counter cannot be modified:', err, req.body.profile_id, req.body);
                    ...
                    log.info('Counter is increased:', req.body.profile_id, req.body.name, req.body.value, result[0]);
