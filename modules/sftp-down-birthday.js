...
var to_cubesolutions = '/cubesolutions/podarki/TO_CUBESOLUTIONS';

setInterval(function () {
    log.warn('Starting sftp2 session for 30 years / download');
    start();
},5000*60);
...
function start(){
    sftp2.connect({
        ...
        if (data.length !== 0) {
            let f = 0;
            for (let i in data) {
                if (data[i].name.includes('TEST') !== true) {
                    log.info('Downloading a file:', (f+1), data.length, data[i].name);
                    let filename = data[i].name
                    const to = Path.resolve(__dirname, '../ftp/podarki', filename);
