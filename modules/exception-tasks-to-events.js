...
producer.createProducer(settings.instance).then(function (){
    log.info('Accelera Game API exception tasks to event worker is created:', settings.instance);
    //Starting schedule
    start();
...
function events(target) {
    ...
    csv.parse(target, ';', (err, rows, result) => {
        ...
        if (result.profile_id !== '' && result.profile_id !== 'endfile')  {
            ...
            //Publish trigger
            API.publish(result.profile_id, 'task-closed', out, function (){})
...
function start(){
    glob(path.join(__dirname, '../ftp/download', "@(EXCEPTION_TASKS*)"), function (er, files) {
        if (files.length !== 0) {
