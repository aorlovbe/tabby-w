...
var from_cubesolutions = '/cubesolutions/podarki/FROM_CUBESOLUTIONS';

setInterval(function () {
    log.info('Starting Birthday SFTP session / upload');
    start();
...
function start(){
    glob(path.join(__dirname, '../ftp/upload_birthday', "!(uploaded*)"), function (er, files) {
        if (files.length !== 0) {
            sftp.connect({
                host: '217.118.84.207',
                port: '22',
                ...
                Promise.each(files, function(file) {
                    ...
                    log.info('Finishing SFTP session');
                    setTimeout(function (){
                        sftp.end();
                    },10000)
                }).catch(function(err) {
