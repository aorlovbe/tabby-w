...
producer.createProducer(settings.instance).then(function () {
  log.info(
    "Accelera Game API tabby-dev tasks to event worker is created:",
    settings.instance
  );
  //Starting schedule
  start();
        ...
        if (parseInt(result.quantity) !== 1) {
          for (let i = 0; i < parseInt(result.quantity); i++) {
            let out = {
              ...
              name: "tabby-dev-task-" + result.task_id,
          ...
          API.publish(
            result.profile_id,
            "task-completed-tabby-dev-task-" + result.task_id,
            out,
            function () {}
          );
...
function start() {
  glob(
    path.join(__dirname, "../ftp/download_tabby-dev", "@(CLOSE*)"),
    function (er, files) {
      if (files.length !== 0) {
