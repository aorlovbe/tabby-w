                    ...
                    if (games[i].game_id === 'rock-paper-scissors') {
                        Leaderboards.getUnmaskedbyDate({"body" : {"game_id" : games[i].game_id, "name" : "points"}}, lastdate, lastmonth, lastweek, function (err, leaderboard) {
                            ...
                            for (let i in monthly.scores) {
                                places.push({"place" : Object.keys(monthly.scores).indexOf(i)+1, "player_id" : i, "score" : monthly.scores[i]})
                            }

                            log.warn('Got places: TOP', monthly_limit, places.slice(0, monthly_limit))
                            ...
                            for (let k in games[i].leaderboard.gifts.monthly ) {
                                if (typeof ( games[i].leaderboard.gifts.monthly[k].place) === 'string') {
                                    reformat_monthly(
                                    ...
                                    monthly_reformatted.push( games[i].leaderboard.gifts.monthly[k])
                                }
                            }
                            ...
                            for (let x in rewarded){
                                ...
                                //Pushing to accelera
                                try {
                                    let reward = monthly_reformatted[rewarded[x].place-1].reward;
                                    let context = {
                                        ...
                                        "details" : monthly_reformatted[rewarded[x].place-1].details
                                    };
                                    log.warn('Processing reward trigger:', JSON.stringify(context));

                                    //Publish trigger
                                    API.publish(rewarded[x].player_id, 'monthly_reward', context, function (){})
