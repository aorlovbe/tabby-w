...
producer.createProducer(settings.instance).then(function (){
    log.info('Accelera Game API tasks to event worker is created:', settings.instance);
    //Starting schedule
    start();
        ...
        if (result.profile_id !== '' && result.profile_id !== 'endfile')  {
            if (parseInt(result.quantity) !== 1) {
                for (let i = 0; i < parseInt(result.quantity); i++) {
                    ...
                    //Publish trigger
                    API.publish(result.profile_id, 'task-completed-task-'+result.task_id, out, function (){})
...
function start(){
    glob(path.join(__dirname, '../ftp/download', "@(CLOSE*)"), function (er, files) {
        if (files.length !== 0) {
